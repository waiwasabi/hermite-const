...
With $N=9$ and $p=64$, find the vectors $a\in\mathfrak{U}(p)$ achieving the maximum value of $\lambda_1(\mathfrak{B}(p, a))$
```
./main 9 64
```

With $N=5$ and *even* $p\in [2, 99]$, find the vectors $a\in\mathfrak{U}(p)$ achieving the maximum value of $\lambda_1(\mathfrak{B}(p, a))$
